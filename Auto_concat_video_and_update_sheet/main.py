...
from module5 import auto_concat
from datetime import datetime
from update_sheet3 import excel_to_sheet
import os
...
OUTPUT_DIR = './output'
MAX_AGE_SECONDS = 137 * 24 * 60 * 60  # Chỉ chọn video trong vòng 137 ngày
...
def generate_video_lists(suitable_df, original_df, durations, last_used, file_paths):
    ...
    for i in range(len(suitable_df)):
        ...
        try:
            first_index = int(suitable_df.iloc[i]['first vids'])-1
        except (ValueError, IndexError):
            print(f"Invalid 'first vids' index: {suitable_df.iloc[i]['first vids']}")
            continue

        first_duration = durations[first_index]

        for list_index in range(num_lists):
            available_indexes = [
                idx for idx in range(len(original_df))
                if idx != first_index and last_used[idx] <= MAX_AGE_SECONDS
            ]
            ...
            selected_files = [file_paths[idx] for idx in selected_indexes]
...
def main():
    ...
    try:
        ...
        print("Đã ghi toàn bộ dữ liệu vào Excel (có cột 'output directory').")
    except Exception as e:
        print(f"Lỗi khi ghi toàn bộ file Excel: {e}")
