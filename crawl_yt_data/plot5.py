...
colors = ['#02acf5', '#f54302', '#00cc96', '#ff6692', '#9467bd', '#e377c2', '#7f7f7f', '#bcbd22']  # Extended for more channels
for i, channel in enumerate(selected_channels):
    channel_subset = channel_data[channel_data['channel_name'] == channel]
    # Views trace
    fig1.add_trace(
        go.Scatter(
            ...
            line=dict(color=colors[i % len(colors)], width=3),  # Thicker line for visibility
            hovertemplate=f'<br>Ngày: %{{x|%Y-%m-%d}}<br>Lượt Xem: %{{y:,d}}',
        ),
        secondary_y=False,
    )
    # Likes trace
    fig1.add_trace(
        go.Scatter(
            ...
            line=dict(color=colors[i % len(colors)], width=2, dash='dashdot'), 
            hovertemplate=f'<br>Ngày: %{{x|%Y-%m-%d}}<br>Lượt Thích: %{{y:,d}}',
        ),
        secondary_y=True,
    )
...
color_map = {channel: colors[i % len(colors)] for i, channel in enumerate(selected_channels)}
